...
def _cleanup_kitty_tmp_images():
    for image_path in _KITTY_TMP_IMAGES:
        if os.path.exists(image_path):
            os.remove(image_path)
        # end if
    # end for
# end def _cleanup_kitty_tmp_images
...
def show_expr(label: str, expr: pm.MathExpr):
    ...
    print(f"{label}: {latex}")
    print(f"{label} shape: {expr.shape}")

    if os.environ.get("KITTY_WINDOW_ID") is None:
        return
    # end if
    ...
    try:
        img = render.render_latex(expr)
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp_file:
            tmp_path = tmp_file.name
        # end with
        img.save(tmp_path)
        _KITTY_TMP_IMAGES.append(tmp_path)

        completed = subprocess.run(["kitten", "icat", "--unicode-placeholder", tmp_path], check=False)
        if completed.returncode != 0:
            fallback = subprocess.run(["kitty", "+kitten", "icat", "--unicode-placeholder", tmp_path], check=False)
            if fallback.returncode != 0:
                print(
                    f"[show_expr] kitty image display failed (kitten={completed.returncode}, kitty={fallback.returncode})"
                )
            # end if
        # end if
    except (OSError, RuntimeError) as exc:
        print(f"[show_expr] Could not display image in kitty: {exc}")
    # end try
    print("")
# end def show_expr
