...
def test_build_tensor_vector_from_scalars():
    ...
    elements = [
        pm.const("builder_a", data=1.5, dtype=pm.DType.FLOAT32),
        2,
        3.25,
    ]
...
def test_build_tensor_with_variables_and_expressions():
    ...
    with pm.new_context():
        state = pm.var("builder_state", dtype=pm.DType.FLOAT32, shape=())
        pm.set_value(state.name, 0.5)

        elements = [
            state,
            pm.const("builder_bias", data=-1, dtype=pm.DType.INT32),
            state + 1.0,
            4.25,
        ]
        expr = B.build_tensor(elements)
...
def test_build_tensor_respects_explicit_shape():
    ...
    elements = [
        pm.const("builder_m0", data=1, dtype=pm.DType.INT32),
        2.0,
        3.0,
        pm.const("builder_m3", data=-4, dtype=pm.DType.FLOAT32),
    ]
...
def test_build_tensor_rejects_non_scalar_inputs():
    ...
    vector = pm.const("builder_vector", data=[1.0, 2.0], dtype=pm.DType.FLOAT32)
