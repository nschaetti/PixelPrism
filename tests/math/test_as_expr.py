...
def test_as_expr_returns_math_expr_unchanged():
    ...
    None
    """
    tensor = Tensor(name="identity", data=np.array([1.0], dtype=np.float32))
...
def test_as_expr_converts_python_scalar_with_dtype_and_mutability():
    ...
    None
    """
    result = as_expr(3.5, dtype=DType.FLOAT32, mutable=True)
    assert isinstance(result, Tensor)
    assert result.dtype is DType.FLOAT32
    assert result.shape.dims == ()
    assert result.mutable
    np.testing.assert_array_equal(result.value, np.array(3.5, dtype=np.float32))
...
def test_as_expr_converts_numpy_scalar_with_dtype_override():
    ...
    None
    """
    result = as_expr(np.float32(2.25), dtype=DType.FLOAT64, mutable=False)
    assert isinstance(result, Tensor)
    assert result.dtype is DType.FLOAT64
    assert result.shape.dims == ()
    assert not result.mutable
    np.testing.assert_array_equal(result.value, np.array(2.25, dtype=np.float64))
...
def test_as_expr_wraps_numpy_array_with_and_without_dtype_override():
    ...
    result = as_expr(data, dtype=DType.FLOAT32, mutable=False)
    assert isinstance(result, Tensor)
    assert result.dtype is DType.FLOAT32
    assert result.shape.dims == (2, 2)
    assert not result.mutable
    np.testing.assert_array_equal(result.value, np.array(data, dtype=np.float32))

    data = np.array([[1, 2], [3, 4]], dtype=np.int64)
    result = as_expr(data, mutable=False)
    assert isinstance(result, Tensor)
    assert result.dtype is DType.INT64
    assert result.shape.dims == (2, 2)
    assert not result.mutable
    np.testing.assert_array_equal(result.value, np.array(data, dtype=np.int64))
...
def test_as_expr_converts_nested_list_with_dtype():
    ...
    assert result.shape.dims == (2, 2)
    np.testing.assert_array_equal(result.value, np.array(data, dtype=np.int32))
...
def test_as_expr_converts_nested_list_default_dtype_float64():
    ...
    assert result.dtype is DType.FLOAT64
    assert result.shape.dims == (2, 2)
    np.testing.assert_array_equal(result.value, np.array(data, dtype=np.float64))
