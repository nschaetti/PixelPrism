...
def test_where_scalar_case():
    cond = pm.const("cond_scalar", data=True, dtype=pm.DType.B)
    x = pm.const("x_scalar", data=1.5, dtype=pm.DType.R)
    y = pm.const("y_scalar", data=-2.0, dtype=pm.DType.R)
    expr = where(cond, x, y)
...
def test_where_vector_case(cond_vals, x_vals, y_vals):
    cond, cond_np = _make_const("cond_vec", cond_vals, pm.DType.B)
    x, x_np = _make_const("x_vec", x_vals, pm.DType.R)
    y, y_np = _make_const("y_vec", y_vals, pm.DType.R)
    expr = where(cond, x, y)
...
def test_where_matrix_case():
    ...
    x_values = [[1, 2], [3, 4]]
    y_values = [[-1, -2], [-3, -4]]
    cond, cond_np = _make_const("cond_mat", cond_values, pm.DType.B)
    x, x_np = _make_const("x_mat", x_values, pm.DType.Z)
    y, y_np = _make_const("y_mat", y_values, pm.DType.Z)
    expr = where(cond, x, y)
    expected = np.where(cond_np, x_np, y_np)
    np.testing.assert_allclose(expr.eval().value, expected)
...
def test_where_dtype_promotion():
    cond = pm.const("cond_promote", data=[True, False], dtype=pm.DType.B)
    x = pm.const("x_promote", data=[1.0, 2.0], dtype=pm.DType.R)
    y = pm.const("y_promote", data=[1, 2], dtype=pm.DType.Z)
    expr = where(cond, x, y)
    assert expr.dtype == pm.DType.R
...
def test_where_cond_not_bool_raises():
    cond = pm.const("cond_bad", data=1, dtype=pm.DType.Z)
    x = pm.const("x_bad", data=1.0, dtype=pm.DType.R)
    y = pm.const("y_bad", data=-1.0, dtype=pm.DType.R)
...
def test_where_shape_mismatch_raises():
    cond = pm.const("cond_shape", data=[True, False], dtype=pm.DType.B)
    x = pm.const("x_shape", data=[[1.0, 2.0], [3.0, 4.0]], dtype=pm.DType.R)
    y = pm.const("y_shape", data=[[0.0, 0.0], [0.0, 0.0]], dtype=pm.DType.R)
...
def test_if_selects_true_branch():
    cond = pm.const("if_cond_true", data=True, dtype=pm.DType.B)
    then_expr = pm.const("if_then", data=2.0, dtype=pm.DType.R)
    else_expr = pm.var("if_else_unbound", dtype=pm.DType.R, shape=())
...
def test_if_selects_false_branch_without_evaluating_true():
    cond = pm.const("if_cond_false", data=False, dtype=pm.DType.B)
    then_expr = pm.var("if_then_unbound", dtype=pm.DType.R, shape=())
    else_expr = pm.const("if_else", data=-5.0, dtype=pm.DType.R)
    expr = if_(cond, then_expr, else_expr)
...
def test_if_requires_matching_shapes():
    cond = pm.const("if_cond", data=True, dtype=pm.DType.B)
    then_expr = pm.const("if_then_shape", data=[1.0, 2.0], dtype=pm.DType.R)
    else_expr = pm.const("if_else_shape", data=3.0, dtype=pm.DType.R)
    with pytest.raises(ValueError):
