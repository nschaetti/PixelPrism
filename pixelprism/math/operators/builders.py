...
class Normal(ParametricBuilder):
    ...
    def __init__(self, shape: Shape, loc: float = 0.0, scale: float = 1.0, dtype: DType = DType.R):
        if scale < 0:
            raise ValueError("Normal requires scale >= 0.")
        # end if
        super().__init__(shape=shape, loc=loc, scale=scale, dtype=dtype)
        self._shape = shape
        self._loc = float(loc)
    ...
    def check_parameters(
        ...
        return shape.n_dims >= 0 and scale >= 0
    # end def check_parameters
    ...
    def _eval(self, operands: Operands, **kwargs) -> Tensor:
        numpy_dtype = to_numpy(self._dtype)
        data = np.random.normal(loc=self._loc, scale=self._scale, size=self._shape.dims)
        return Tensor(data=np.asarray(data, dtype=numpy_dtype), dtype=self._dtype)
    # end def _eval
    ...
    def __str__(self) -> str:
        return (
            f"{self.NAME}(shape={self._shape.dims}, loc={self._loc}, "
            f"scale={self._scale}, dtype={self._dtype.name})"
        )
    # end def __str__
...
class Uniform(ParametricBuilder):
    ...
    def __init__(self, shape: Shape, low: float = 0.0, high: float = 1.0, dtype: DType = DType.R):
        if high <= low:
    ...
    def check_parameters(
        ...
        return shape.n_dims >= 0 and high > low
    # end def check_parameters
    ...
    def _eval(self, operands: Operands, **kwargs) -> Tensor:
        numpy_dtype = to_numpy(self._dtype)
        data = np.random.uniform(low=self._low, high=self._high, size=self._shape.dims)
        return Tensor(data=np.asarray(data, dtype=numpy_dtype), dtype=self._dtype)
    # end def _eval
    ...
    def __str__(self) -> str:
        return (
            f"{self.NAME}(shape={self._shape.dims}, low={self._low}, "
            f"high={self._high}, dtype={self._dtype.name})"
        )
    # end def __str__
...
class RandInt(ParametricBuilder):
    ...
    def __init__(self, shape: Shape, low: int, high: Optional[int] = None, dtype: DType = DType.Z):
        if high is None:
            if low <= 0:
        ...
        self._shape = shape
        self._low = int(low)
        self._high = int(high) if high is not None else None
        self._dtype = dtype
    # end def __init__
    ...
    def _eval(self, operands: Operands, **kwargs) -> Tensor:
        numpy_dtype = to_numpy(self._dtype)
        data = np.random.randint(low=self._low, high=self._high, size=self._shape.dims)
        return Tensor(data=np.asarray(data, dtype=numpy_dtype), dtype=self._dtype)
    # end def _eval
...
class Poisson(ParametricBuilder):
    ...
    def __init__(self, shape: Shape, lam: float = 1.0, dtype: DType = DType.Z):
        if lam < 0:
            raise ValueError("Poisson requires lam >= 0.")
        # end if
        super().__init__(shape=shape, lam=lam, dtype=dtype)
        self._shape = shape
        self._lam = float(lam)
        self._dtype = dtype
    # end def __init__
    ...
    @classmethod
    def check_parameters(cls, shape: Shape, lam: float = 1.0, dtype: DType = DType.Z) -> bool:
        return shape.n_dims >= 0 and lam >= 0
    # end def check_parameters
    ...
    def _eval(self, operands: Operands, **kwargs) -> Tensor:
        numpy_dtype = to_numpy(self._dtype)
        data = np.random.poisson(lam=self._lam, size=self._shape.dims)
        return Tensor(data=np.asarray(data, dtype=numpy_dtype), dtype=self._dtype)
    # end def _eval
    ...
    def __str__(self) -> str:
        return f"{self.NAME}(shape={self._shape.dims}, lam={self._lam}, dtype={self._dtype.name})"
    # end def __str__
...
class Bernoulli(ParametricBuilder):
    ...
    def __init__(self, shape: Shape, p: float = 0.5, dtype: DType = DType.Z):
        if p < 0.0 or p > 1.0:
            raise ValueError("Bernoulli requires p in [0, 1].")
        # end if
        super().__init__(shape=shape, p=p, dtype=dtype)
        self._shape = shape
        self._p = float(p)
        self._dtype = dtype
    # end def __init__
    ...
    @classmethod
    def check_parameters(cls, shape: Shape, p: float = 0.5, dtype: DType = DType.Z) -> bool:
        return shape.n_dims >= 0 and 0.0 <= p <= 1.0
    # end def check_parameters
    ...
    def _eval(self, operands: Operands, **kwargs) -> Tensor:
        numpy_dtype = to_numpy(self._dtype)
        data = np.random.binomial(n=1, p=self._p, size=self._shape.dims)
        return Tensor(data=np.asarray(data, dtype=numpy_dtype), dtype=self._dtype)
    # end def _eval
    ...
    def __str__(self) -> str:
        return f"{self.NAME}(shape={self._shape.dims}, p={self._p}, dtype={self._dtype.name})"
    # end def __str__
